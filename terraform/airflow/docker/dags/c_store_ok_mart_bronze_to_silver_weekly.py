...
from sql.c_store_ok_mart_queries import (
...
# Create the DAG
with DAG(
    'c_store_ok_mart_bronze_to_silver_weekly',
    default_args=default_args,
    description='C store ok mart data transfer from bronze to silver (one date at a time)',
    schedule_interval= SCHEDULE_INTERVALS['weekly'],  # Run at 10 AM every Saturday on Taiwan time
    start_date=pendulum.datetime(2023, 1, 1, tz="UTC"),
    catchup=False,
    tags=['c_store', 'ok_mart', 'bronze', 'silver', 'incremental-load'],
