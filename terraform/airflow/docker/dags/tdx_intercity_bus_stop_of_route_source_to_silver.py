...
def fetch_bus_stop_of_route_to_gcs(**context):
    ...
    print("Fetching bus stop of route from real TDX API...")
    data = get_tdx_data(app_id, app_key, auth_url, url)
    
    file_name = f"bus/bronze/inter_city_bus_stop_of_route/inter_city_bus_stop_of_route_{execution_year_month}.json"
...
def process_bus_stop_of_route_to_staging(**context):
    ...
    table_schema = [
        ...
        {'name': 'operators', 'type': 'RECORD', 'mode': 'REPEATED', 'fields': [
            ...
            {'name': 'operator_code', 'type': 'STRING', 'mode': 'NULLABLE'},
            {'name': 'operator_no', 'type': 'STRING', 'mode': 'NULLABLE'},
        ]},
        {'name': 'sub_route_uid', 'type': 'STRING', 'mode': 'REQUIRED'},
        {'name': 'sub_route_id', 'type': 'STRING', 'mode': 'REQUIRED'},
        {'name': 'sub_route_name', 'type': 'RECORD', 'mode': 'REQUIRED', 'fields': [
            {'name': 'zh_tw', 'type': 'STRING', 'mode': 'REQUIRED'},
            {'name': 'en', 'type': 'STRING', 'mode': 'NULLABLE'},
        ]},
    ...
    gdf.to_gbq(
        destination_table="bus_silver.inter_city_bus_stop_of_route_staging",
        project_id=project_id,
        credentials=credentials,
        if_exists='replace',
        table_schema=table_schema # Use the defined schema
    )
    print("Successfully loaded data into bus_silver.inter_city_bus_stop_of_route_staging.")

with DAG(
    dag_id="tdx_intercity_bus_stop_of_route_source_to_silver",
    start_date=pendulum.datetime(2023, 1, 1, tz="UTC"),
    schedule="@monthly",
    catchup=False,
    tags=["bus", "silver", "tdx", "inter_city_bus_stop_of_route"],
    default_args={
    ...
    fetch_bronze_data = PythonOperator(
        task_id="fetch_bus_stop_of_route_to_gcs",
        python_callable=fetch_bus_stop_of_route_to_gcs,
    )
