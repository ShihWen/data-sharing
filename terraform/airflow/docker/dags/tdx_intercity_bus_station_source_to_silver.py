...
def fetch_bus_station_to_gcs(**context):
    ...
    url = "https://tdx.transportdata.tw/api/basic/v2/Bus/Station/InterCity?%24format=JSON"
           
    print("Fetching bus station from real TDX API...")
    data = get_tdx_data(app_id, app_key, auth_url, url)
    
    file_name = f"bus/bronze/inter_city_bus_station/inter_city_bus_station_{execution_year_month}.json"
...
def process_bus_station_to_staging(**context):
    ...
    table_schema = [
        ...
        {'name': 'station_geography', 'type': 'GEOGRAPHY', 'mode': 'NULLABLE'},
    ]
    
    gdf.to_gbq(
        destination_table="bus_silver.inter_city_bus_station_staging",
        project_id=project_id,
        credentials=credentials,
        if_exists='replace',
        table_schema=table_schema # Use the defined schema
    )
    print("Successfully loaded data into bus_silver.inter_city_bus_station_staging.")
